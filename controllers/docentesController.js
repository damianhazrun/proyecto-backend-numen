...
exports.create = async (req, res) => {
  ...
  if (age < 18) {
    return res
      .status(400)
      .json({ message: "El docente debe ser mayor de 18 años." });
  }
...
/* Crear un nuevo docente con datos aleatorios con FAKER*/
exports.createRandomDocente = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      message: "Nuevo docente aleatorio creado con éxito",
      data: docente,
    });
  } catch (error) {
    res
      .status(400)
      .json({ message: "Error al crear docente aleatorio: " + error.message });
  }
};
